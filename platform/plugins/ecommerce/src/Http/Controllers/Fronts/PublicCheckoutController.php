...
class PublicCheckoutController extends BaseController
{
    ...
    public function getCheckout(
        ...
        if (! EcommerceHelper::isEnabledGuestCheckout() && ! auth('customer')->check()) {
            return $this
                ->httpResponse()
                ->setNextUrl(route('customer.access.otp.send'));
        }
        ...
        if (empty($customer->email) || empty($customer->name)) {
            return $this
                ->httpResponse()
                ->setMessage(__('Please complete your profile'))
        ...
        if (EcommerceHelper::isEnabledSupportDigitalProducts() && ! EcommerceHelper::canCheckoutForDigitalProducts(
            ...
            return $this
                ->httpResponse()
                ->setError()
                ->setNextUrl(route('customer.access.otp.form'))
    ...
    public function getCheckoutSuccess(string $token)
    {
        ...
        if ($order->payment?->payment_channel?->getValue() == 'gateway') {
            return $this
                ->httpResponse()
                ->setNextUrl(route('customer.payment', [
                    'order_id' => $order->getKey(),
                ]));
        }
